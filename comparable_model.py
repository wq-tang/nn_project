...
class complex_net(alexNet):
    ...
    def build_complex_CNN_for_mnist(self):
        with tf.variable_scope('model_%d'%self.seed):
            conv1 = self.complex_convLayer(self.X_com, [5, 5], [1, 1], 32, "conv1", "SAME",relu_fun = self.relu_fun)
            pool1 = self.complex_maxPoolLayer(conv1,[2, 2],[ 2,2], "pool1", "SAME")

            conv2 = self.complex_convLayer(pool1, [3, 3], [1, 1], 16, "conv2",'SAME',relu_fun = self.relu_fun)
            ...
            fc1 = self.complex_fcLayer([R,I], dim, 128,  name = "fc4",relu_fun = tf.nn.relu)
            self.fc2 = self.complex_fcLayer(fc1, 128, self.CLASSNUM,name =  "fc5",relu_fun = tf.nn.relu)
            self.out = self.fc2
            self.out = tf.sqrt(tf.square(self.out[0])+tf.square(self.out[1]))

    def build_real_CNN_for_mnist(self):
        with tf.variable_scope('model_%d'%self.seed):
            conv1 = self.convLayer(self.X, [5, 5], [1, 1], int(32*1.41)+1, "conv1", "SAME")
            pool1 = self.maxPoolLayer(conv1,[2, 2],[ 2,2], "pool1", "SAME")

            conv2 = self.convLayer(pool1, [3, 3], [1, 1], int(16*1.41)+1, "conv2",'SAME')
            ...
            fc1 = self.fcLayer(Res, mul, int(128*1.41)+1,  name = "fc4")
            self.fc2 = self.fcLayer(fc1, int(128*1.41)+1, self.CLASSNUM, name =  "fc5")
            self.out = self.fc2
    ...
    def build_complex_CNN_for_cifar10(self):
        with tf.variable_scope('model_%d'%self.seed):
            ...
            I = tf.reshape(cnnout[1],[-1,mul])
            dim = R.get_shape()[1].value
            fc1 = self.complex_fcLayer([R,I], dim, 384,  name = "fc4",relu_fun = tf.nn.relu)
            fc2 = self.complex_fcLayer(fc1, 384, 192, name =  "fc5",relu_fun = tf.nn.relu)
            fc3 = self.complex_fcLayer(fc2, 192, self.CLASSNUM, name =  "fc6",norm=False,relu_fun = tf.nn.relu)
            self.out = fc3
            self.out = tf.sqrt(tf.square(self.out[0])+tf.square(self.out[1]))
