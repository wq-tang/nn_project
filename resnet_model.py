...
class Resnet(base_class):
	...
	def bottleneck(self,inputs,kernel_size,depth_bottleneck,scope=None):
		with tf.variable_scope(scope):
			...
			if depth_bottleneck == depth_in:
				...
				shortcut = self.conv(preact,[1,1],[2,2],depth_bottleneck,name = 'shortcut',padding = "SAME")

			residual = self.conv(preact,[kernel_size,kernel_size],[1,1],depth_bottleneck,padding = 'SAME',name='conv1')
	...
	def build_compare_resnet(self,model_num=1):
		...
		with tf.variable_scope('resnet'):
			...
			net = self.connect(net,dim,self.classNum,name='fc-100')
			if self.is_complex:
				return tf.sqrt(tf.square(net[0])+tf.square(net[1]))
			return net
