...
def Secondary_net(model_name,is_complex):
	...
	test_data = []
	for i in range(4):
		batchTrain, batchTest = read_mnist(model_name+'_'+tail+str(i+1)+'.h5',batch_step,1000)
	...
	w4 = tf.get_variable("w4", shape = [1, class_num], dtype = tf.float32,\
                    initializer = tf.contrib.layers.xavier_initializer( uniform=True, seed=seed,dtype=tf.float32))
	out = tf.multiply(x1,w1)+tf.multiply(x2,w2)+tf.multiply(x3,w3)+tf.multiply(x4,w4)
